...
% Wilson     parameters for Ethanol (1) - Cyclohexane (2) 
alpha0 = 1; % alpha
A0 = [-0.3015;-0.4121]; % Binary interaction parameters (Constant)
B0 = [-0.2185;-0.2845]; % Binary interaction parameters (T-dependent)

% Wilson (S) parameters for Ethanol (1) - Cyclohexane (2)
% alpha0 = -1.3352; % alpha
% A0 = [0.7656;-0.0159]; % Binary interaction parameters (Constant)
% B0 = [0.0840;0.2052]; % Binary interaction parameters (T-dependent)
...
L2  = (k7*L1); % Eq. 16
L3  = 1./(k8*L2); % Eq. 17
L4  = k8*L3-k9; % Eq. 18  - (GE/RT)/xi
L5  = (k8*L3); % Eq. 19
L6  = (-k10*W) + (k6*L2); % Eq. 20
L8  = (k6*x).*(k6*L3).*(k6*L5).*(k5*L6); % Eq. 25
