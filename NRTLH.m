...
betaalphas  = [0;-0.6885;-0.6885;0];    % Non-randomness factor
betaW       = [0;0.8256;0.1638;0];      % Binary interaction parameters (Constant)
...
L4          = (MB*L3);                                                  % Eq. (39)
L5          = 1./(I1*L2);                                               % Eq. (40)
L6          = tanh((I1*L4).*(I1*L5));                                   % Eq. (41) - hyperbolic tangent transfer function
L7          = (I1*L5);                                                  % Eq. (42)
L8          = (MC*LW).*(MA*L4);                                         % Eq. (43)
L9          = (MC*LV).*(MA*L2);                                         % Eq. (44)
L10         = (-I2*L8) + (I2*L9);                                       % Eq. (45)
L11         = sech((I1*L4).*(I1*L5)).^2;                                % Eq. (46) - derivative of the hyperbolic tangent transfer function
L12         = (MA*x).*(MA*L5).*(MA*L7).*(I2*L10).*(MA*L11);             % Eq. (47)
