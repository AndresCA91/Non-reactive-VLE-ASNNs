...
n_models                      = 5;   % Number of ASNN-based models that will be generated (only the best one will be chosen)
ShowWindow                    = 1;   % Show the neural network interface - 1 for yes and 0 for no
epochs                        = 100; % Maximum number of iterations that the optimizer will perform
Model                         = 'NRTL_NN'; % "NRTL_NN" - MLP or neural network parametrization
...
tvdT                          = [t1dT];             % Preparation of the derivative of the temperature dependent vector 
...
W                             = {abs(1./Pi.^2)';abs(1./(1./P).^2)';abs(1./y.^2)';5*abs(1./HEXRT2n'.^2)}; % Weights used to transform the shape of mean-square-error optimized to an average absolute relative deviation
...
q                             = 2; % Number of stacked tanh functions
[k1,k2,k3,k4,k5,k6,k7,k8,k9,k10,k11,k12,k13,k14,k15,k16] = AuxMatrices_NN(n,p,q); % Auxiliary matrices of fixed parameters

%% Routines that construct the ASNNs

run('net_VLEHEX_NRTL_NN')

%% Training options

net.initFcn                   = 'initlay'; % Basic Matlab function for neural networks
net.performFcn                = 'mse'; % Mean square error performance function
net.trainFcn                  = 'trainbr'; % trainlm (Levenberg-Marquardt) or Bayesian regularization (trainbr)
