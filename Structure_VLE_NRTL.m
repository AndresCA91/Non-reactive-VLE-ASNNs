...
% NRTL parameters for Ethanol (1) - Cyclohexane (2) 
alpha0 = [0.45;0.45]; % alpha
A0 = [-0.6496;1.1809]; % Binary interaction parameters (Constant)
B0 = [0.6107;0.3698]; % Binary interaction parameters (T-dependent)

%% Fixed Model Parameters / Auxiliary Matrices

n  = length(A0); % Number of components
p  = size(B0,2); % Number of temperature dependent parameters
...
xlabel('{\itx_1},{\ity_1}')
ylabel('{\itP} / kPa')
legend([A,B],'Liquid phase', 'Vapor phase','Location','Southwest','NumColumns',1)
set(gca,'FontSize',18)
